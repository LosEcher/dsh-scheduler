...
export function nextCronAfter(cron, afterMs, tz) {
  ...
  for (;;) {
    ...
    // Weekday must be computed in the *job timezone*, not UTC: for tz with a
    // positive offset (e.g. Asia/Shanghai) the local date can lag the UTC date
    // inside the 00:00–offset window, so a UTC weekday mismatches `d`/`mo`
    // (local calendar) and day-of-week-restricted crons (e.g. `0 9 * * 1`)
    // would never fire. Shift the instant back to local wall-clock first.
    const weekday = new Date(t + tzOffsetMinutes(tz, t) * 60000).getUTCDay()
